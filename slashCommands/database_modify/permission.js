...
module.exports = {
    ...
    help: {
        usage: '/{command} [command] [command-type] [permission] (group_name)/(role)',
    },
	options: [
        {
            name: 'command',
    ...
    run: async (client, interaction) => {
        ...
        if (client.config['database.settings'].enabled) {          
            ...
            if (argpermission === 'owners' && client.config.owners.includes(String(interaction.user.id))) {
                const userNotOwner = new EmbedBuilder()
                .setDescription(lang('commands:permission:not_owner', server.language, [argcommand]))
                .setColor('Red');
                return interaction.reply({embeds: [userNotOwner], ephemeral: true});
            }

            if (!server.commands_data.some(c => c.name === argcommand && c.permission === 'owners') && client.config.owners.includes(String(interaction.user.id))) {
                const userNotOwner = new EmbedBuilder()
                .setDescription(lang('commands:permission:command_only_owner', server.language, [argcommand]))
                .setColor('Red');
                return interaction.reply({embeds: [userNotOwner], ephemeral: true});
            }
            ...
            client.commands.forEach((command) => {
                if (!(server.commands_data.some(c => c.name === command.name)))
                {
                    new_commands_data.push({
                        ...
                        permission: (command.name == argcommand) ? ((argpermission == 'null') ? {type: null} : (argpermission == 'group') ? {type: 'group', group: arggroupname} : (argpermission == 'role') ? {type: 'role', role:String(argrole.id)} : {type: argpermission}) : command.permissions.user_permission
                    });    
                }
            });
            ...
            server.commands_data.forEach((command) => {
                if (client.commands.has(command.name))
                {
                    new_commands_data.push({
                        ...
                        type: 'command',
                        enabled: command.enabled,
                        permission: (command.name == argcommand) ? ((argpermission == 'null') ? {type: null} : (argpermission == 'group') ? {type: 'group', group: arggroupname} : (argpermission == 'role') ? {type: 'role', role:String(argrole.id)} : {type: argpermission}) : command.permission
                    });    
            ...
            .setDescription(lang('commands:permission:changed', server.language, [argcommand]))
            .setColor('Green');
