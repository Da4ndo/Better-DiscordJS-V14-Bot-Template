	...
	options: [
        {
            ...
            options: [
                {
                    name: 'category',
                    description: 'Specify the category where you want to be ticket channels created.',
                    type: 7,
                    required: true
                }
            ]
            ...
            if (server.enabled_managers.includes(manager.name)) {
                ...
                try {
                    ...
                    if (client.config['database.settings'].enabled) {
                        ...
                        client.config['universal.settings'].manager[manager.name].settings.category_id = String(category.id);
                    ...
                    .setTitle('Ticket')
                    .setDescription(lang('commands:ticket:setup', server.language, [interaction.guild.name]))
                    .setColor('#EEEEEC');   
                    ...
                    await interaction.channel.send({embeds: [ticketembed], components: [new ActionRowBuilder().addComponents(ticketbutton)]});
                    await interaction.reply({content: lang('commands:setup:succesful', server.language, ['ticket']), ephemeral: true});
